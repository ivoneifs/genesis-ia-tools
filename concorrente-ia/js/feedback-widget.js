...
const FeedbackWidget = (function() {
  const TOOL_SLUG = 'concorrente-ia';
  ...
  function createWidget() {
    ...
    btn.style.cssText = 'position:fixed;bottom:24px;right:24px;z-index:9999;display:flex;align-items:center;gap:6px;padding:11px 18px;background:#111;color:#fff;border:none;border-radius:0;font-size:12px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;font-family:inherit;cursor:pointer;transition:background .15s;';
    btn.addEventListener('mouseenter', function() { btn.style.background = '#e2231a'; });
    btn.addEventListener('mouseleave', function() { btn.style.background = '#111'; });
  ...
  function showModal() {
    ...
    modal.innerHTML =
      '<div id="feedback-overlay" style="position:absolute;inset:0;background:rgba(17,17,17,.6);"></div>' +
      '<div style="position:relative;background:#fff;border:1.5px solid #111;border-radius:0;padding:28px;width:90%;max-width:440px;">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;border-bottom:1.5px solid #111;padding-bottom:12px;">' +
          '<h3 style="font-size:16px;font-weight:700;letter-spacing:.02em;text-transform:uppercase;color:#111;">Enviar Feedback</h3>' +
          '<button id="feedback-close" style="background:none;border:1px solid #d8d8d4;border-radius:0;color:#111;padding:4px 10px;cursor:pointer;font-size:16px;">&times;</button>' +
        '</div>' +
        '<div style="display:flex;gap:8px;margin-bottom:16px;">' +
          '<button class="fb-type-btn" data-type="bug" style="flex:1;padding:10px;background:#fff;border:1px solid #d8d8d4;border-radius:0;color:#666;font-size:12px;font-weight:700;letter-spacing:.03em;text-transform:uppercase;cursor:pointer;font-family:inherit;">Bug</button>' +
          '<button class="fb-type-btn active" data-type="feature" style="flex:1;padding:10px;background:#111;border:1px solid #111;border-radius:0;color:#fff;font-size:12px;font-weight:700;letter-spacing:.03em;text-transform:uppercase;cursor:pointer;font-family:inherit;">Sugestão</button>' +
        '</div>' +
        '<textarea id="feedback-text" placeholder="Descreva o bug ou sua sugestão..." style="width:100%;min-height:120px;padding:14px;background:#fafaf8;border:1px solid #d8d8d4;border-radius:0;color:#111;font-size:14px;font-family:inherit;resize:vertical;outline:none;"></textarea>' +
        '<button id="feedback-send" style="width:100%;margin-top:12px;padding:14px;background:#111;color:#fff;border:none;border-radius:0;font-size:13px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;cursor:pointer;font-family:inherit;">' +
