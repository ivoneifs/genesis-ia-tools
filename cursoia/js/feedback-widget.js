...
const FeedbackWidget = (function() {
  ...
  function createWidget() {
    ...
    btn.style.cssText = 'position:fixed;bottom:24px;right:24px;z-index:9999;display:flex;align-items:center;gap:6px;padding:10px 18px;background:#1b3a2b;color:#f3efe4;border:1px solid #b08d3e;border-radius:100px;font-size:13px;font-weight:600;font-family:inherit;cursor:pointer;box-shadow:0 4px 20px rgba(27,58,43,.3);transition:transform .2s,box-shadow .2s;';
  ...
  function showModal() {
    ...
    modal.innerHTML =
      ...
      '<div style="position:relative;background:#f3efe4;border:1px solid #1b3a2b;border-radius:6px;padding:28px;width:90%;max-width:440px;">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">' +
          '<h3 style="font-size:18px;font-weight:700;color:#1b3a2b;font-family:Iowan Old Style,Palatino,Georgia,serif;">Enviar Feedback</h3>' +
          '<button id="feedback-close" style="background:none;border:1px solid #1b3a2b;border-radius:4px;color:#1b3a2b;padding:6px 10px;cursor:pointer;font-size:16px;">&times;</button>' +
        '</div>' +
        '<div style="display:flex;gap:8px;margin-bottom:16px;">' +
          '<button class="fb-type-btn" data-type="bug" style="flex:1;padding:10px;background:rgba(176,141,62,.08);border:1px solid rgba(27,58,43,.2);border-radius:4px;color:#8a5a2b;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">Bug</button>' +
          '<button class="fb-type-btn active" data-type="feature" style="flex:1;padding:10px;background:rgba(63,107,78,.12);border:1px solid #3f6b4e;border-radius:4px;color:#1b3a2b;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">Sugestão</button>' +
        '</div>' +
        '<textarea id="feedback-text" placeholder="Descreva o bug ou sua sugestão..." style="width:100%;min-height:120px;padding:14px;background:#fff;border:1px solid rgba(27,58,43,.25);border-radius:4px;color:#1b3a2b;font-size:14px;font-family:inherit;resize:vertical;outline:none;"></textarea>' +
        '<button id="feedback-send" style="width:100%;margin-top:12px;padding:14px;background:#1b3a2b;color:#f3efe4;border:none;border-radius:4px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;">' +
    ...
    modal.querySelectorAll('.fb-type-btn').forEach(function(b) {
      b.addEventListener('click', function() {
        modal.querySelectorAll('.fb-type-btn').forEach(function(x) {
          ...
          x.style.borderColor = 'rgba(27,58,43,.2)';
        });
        b.style.background = b.dataset.type === 'bug' ? 'rgba(176,141,62,.08)' : 'rgba(63,107,78,.12)';
        b.style.borderColor = b.dataset.type === 'bug' ? 'rgba(27,58,43,.2)' : '#3f6b4e';
