...
const ApiKeyManager = (function() {
  const STORAGE_PREFIX = 'quiz-ia_apikey_';
  ...
  const ENABLED_SERVICES = ['openrouter', 'openai']; // OpenRouter preferred (one key → every model)
  ...
  // Refreshed against the live OpenRouter ranking on 2026-06-24 (build day).
  const AI_MODELS = {
    claude: {
    ...
    trending: {
      ...
      // Top movers from the live OpenRouter ranking on build day (2026-06-24).
      models: [
