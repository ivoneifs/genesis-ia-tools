...
const ApiKeyManager = (function() {
  const STORAGE_PREFIX = 'sequencia-ia_apikey_';
  ...
  // OpenRouter first (one key → every model), then native OpenAI, then native
  // Google Gemini (free AI Studio tier). getActiveKey() prefers the first present.
  ...
  // Refreshed from the LIVE OpenRouter ranking on 2026-07-07 (every id verified to
  // exist in /api/v1/models). Route through OpenRouter (provider-prefixed ids) so
  ...
  const AI_MODELS = {
    ...
    trending: {
      ...
      // Live OpenRouter trending — refreshed 2026-07-07.
      models: [
  ...
  function renderModelPicker(selectId) {
    ...
    // Attach the change listener ONCE per <select> (renderModelPicker may be
    // called repeatedly — e.g. each time the keys modal opens). A tool can host
    // more than one model picker (compositor + modal); keep them in sync.
    if (!select._pickerWired) {
      select._pickerWired = true;
      select.setAttribute('data-model-picker', '1');
      select.addEventListener('change', function() {
        setModel(select.value);
        document.querySelectorAll('select[data-model-picker]').forEach(function(other) {
          if (other !== select && other.value !== select.value) other.value = select.value;
        });
      });
    }
  }
  ...
  function setupModal() {
    ...
    if (manageBtn && modal) {
      manageBtn.addEventListener('click', function() {
        ...
        renderModelPicker('model-select-modal');
