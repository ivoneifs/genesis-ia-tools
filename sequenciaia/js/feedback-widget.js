 ...
 * Themed to the Airmail Dispatch direction (postal navy / airmail red / par-avion blue).
 */
const FeedbackWidget = (function() {
  const TOOL_SLUG = 'sequencia-ia';
  ...
  function createWidget() {
    ...
    btn.style.cssText = 'position:fixed;bottom:24px;right:24px;z-index:9999;display:flex;align-items:center;gap:6px;padding:10px 18px;background:#c8102e;color:#fff;border:none;border-radius:100px;font-size:13px;font-weight:700;font-family:inherit;cursor:pointer;box-shadow:0 4px 16px rgba(200,16,46,.28);transition:transform .2s,box-shadow .2s;';
  ...
  function showModal() {
    ...
    modal.innerHTML =
      '<div id="feedback-overlay" style="position:absolute;inset:0;background:rgba(28,43,74,.5);backdrop-filter:blur(3px);"></div>' +
      '<div style="position:relative;background:#fbf7ee;border:1px solid #c2b18b;border-radius:10px;padding:28px;width:90%;max-width:440px;box-shadow:0 20px 50px -20px rgba(28,43,74,.5);">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">' +
          '<h3 style="font-size:18px;font-weight:800;color:#1c2b4a;font-family:\'Arial Narrow\',sans-serif;text-transform:uppercase;letter-spacing:.03em;">Enviar Feedback</h3>' +
          '<button id="feedback-close" style="background:none;border:1px solid #c2b18b;border-radius:5px;color:#46506a;padding:6px 10px;cursor:pointer;font-size:16px;">&times;</button>' +
        '</div>' +
        '<div style="display:flex;gap:8px;margin-bottom:16px;">' +
          '<button class="fb-type-btn" data-type="bug" style="flex:1;padding:10px;background:rgba(200,16,46,.08);border:1px solid rgba(200,16,46,.25);border-radius:6px;color:#c8102e;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;">Bug</button>' +
          '<button class="fb-type-btn active" data-type="feature" style="flex:1;padding:10px;background:rgba(47,90,168,.12);border:1px solid rgba(47,90,168,.35);border-radius:6px;color:#24467f;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;">Sugestão</button>' +
        '</div>' +
        '<textarea id="feedback-text" placeholder="Descreva o bug ou sua sugestão..." style="width:100%;min-height:120px;padding:14px;background:#fffdf8;border:1px solid #c2b18b;border-radius:6px;color:#1c2b4a;font-size:14px;font-family:inherit;resize:vertical;outline:none;"></textarea>' +
        '<button id="feedback-send" style="width:100%;margin-top:12px;padding:14px;background:#c8102e;color:#fff;border:none;border-radius:6px;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;">' +
    ...
    modal.querySelectorAll('.fb-type-btn').forEach(function(b) {
      b.addEventListener('click', function() {
        modal.querySelectorAll('.fb-type-btn').forEach(function(x) {
          x.style.background = x.dataset.type === 'bug' ? 'rgba(200,16,46,.04)' : 'rgba(47,90,168,.05)';
          x.style.borderColor = '#c2b18b';
          x.style.color = '#46506a';
        });
        b.style.background = b.dataset.type === 'bug' ? 'rgba(200,16,46,.08)' : 'rgba(47,90,168,.12)';
        b.style.borderColor = b.dataset.type === 'bug' ? 'rgba(200,16,46,.25)' : 'rgba(47,90,168,.35)';
        b.style.color = b.dataset.type === 'bug' ? '#c8102e' : '#24467f';
