...
const FeedbackWidget = (function() {
  ...
  function createWidget() {
    ...
    btn.style.cssText = 'position:fixed;bottom:24px;right:24px;z-index:9999;display:flex;align-items:center;gap:6px;padding:10px 18px;background:linear-gradient(135deg,#f5b740,#ffd479);color:#fff;border:none;border-radius:100px;font-size:13px;font-weight:600;font-family:inherit;cursor:pointer;box-shadow:0 4px 20px rgba(245,183,64,.3);transition:transform .2s,box-shadow .2s;';
    ...
    modal.innerHTML =
      ...
      '<div style="position:relative;background:#13161f;border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:28px;width:90%;max-width:440px;">' +
        ...
        '<div style="display:flex;gap:8px;margin-bottom:16px;">' +
          ...
          '<button class="fb-type-btn active" data-type="feature" style="flex:1;padding:10px;background:rgba(245,183,64,.12);border:1px solid rgba(245,183,64,.3);border-radius:10px;color:#f5b740;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">💡 Sugestão</button>' +
        '</div>' +
        ...
        '<button id="feedback-send" style="width:100%;margin-top:12px;padding:14px;background:linear-gradient(135deg,#f5b740,#ffd479);color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;">' +
    ...
    modal.querySelectorAll('.fb-type-btn').forEach(function(b) {
      b.addEventListener('click', function() {
        modal.querySelectorAll('.fb-type-btn').forEach(function(x) {
          x.style.background = x.dataset.type === 'bug' ? 'rgba(239,68,68,.05)' : 'rgba(245,183,64,.05)';
          x.style.borderColor = 'rgba(255,255,255,.08)';
        });
        b.style.background = b.dataset.type === 'bug' ? 'rgba(239,68,68,.1)' : 'rgba(245,183,64,.12)';
        b.style.borderColor = b.dataset.type === 'bug' ? 'rgba(239,68,68,.2)' : 'rgba(245,183,64,.3)';
