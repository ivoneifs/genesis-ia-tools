 ...
 * Keys never leave the user's browser. Never sent to any server (Hard Rules #1/#7/#12).
 *
 * Thumbnail IA uses TWO BYOK keys:
 *   • a TEXT model key (OpenRouter preferred — one key reaches every LLM) to write
 *     the high-CTR headline + a per-thumbnail English visual prompt for the scene;
 *   • an IMAGE key to turn each visual prompt into a REAL generated scene — the
 *     member chooses the provider via the image-model picker: fal.ai (FLUX etc.)
 *     or OpenAI (GPT Image, the same OpenAI key that can also do text).
 * All are the END-USER's own keys — the company key is never shipped and no key is
 * ever transmitted to our backend.
 */
const ApiKeyManager = (function() {
  const STORAGE_PREFIX = 'thumbnail-ia_apikey_';
  ...
  const AI_SERVICES = {
    ...
    fal: {
      name: 'FAL.AI',
      // Real fal keys look like "<uuid>:<hex>". No fixed prefix.
  ...
  // headline + visual prompt; the image key (fal OR openai, per the selected image
  // model) renders the real scene. All are BYOK.
  ...
  // Refreshed + every id re-verified against the live OpenRouter API on
  // 2026-07-14 (build day) — all 13 ids returned HTTP 200 on /models/<id>/endpoints.
  const AI_MODELS = {
    claude: {
  ...
  //
  // The scene is generated WITHOUT text — the high-CTR headline is composited on a
  // <canvas> afterwards — so photographic/illustration models (flux) are ideal.
  // ⚠ REFRESH PER BUILD: fal adds text-to-image models weekly. Refreshed 2026-07-14.
  const IMAGE_MODELS = {
    image: {
      ...
      models: [
        ...
        { id: 'fal-ai/ideogram/v3/quality', name: 'Ideogram V3 — cenas nítidas' }
      ]
    },
    openaiImage: {
      label: 'OpenAI (usa a chave OpenAI)',
      // gpt-image-1 family ONLY: they always return b64_json (data-URI pipeline).
      // dall-e-3 was dropped — the Images API rejects `response_format` now, so its
      // URL-only output can't cross our CSP / would taint the canvas (COR-040).
      models: [
  ...
  const IMAGE_MODEL_STORAGE_KEY = STORAGE_PREFIX + 'fal_model';
  ...
  // COR-041: call this for EVERY model <select> in the DOM (studio, key screen, modal).
  ...
  function getImageModel() {
    try { return localStorage.getItem(IMAGE_MODEL_STORAGE_KEY) || DEFAULT_IMAGE_MODEL; }
    catch { return DEFAULT_IMAGE_MODEL; }
  }

  function setImageModel(modelId) {
    try { if (modelId) localStorage.setItem(IMAGE_MODEL_STORAGE_KEY, modelId); }
    catch { /* localStorage unavailable */ }
  }

  function renderImageModelPicker(selectId, groups) {
    const select = document.getElementById(selectId || 'image-model-select');
    if (!select) return;
    select.innerHTML = '';
    ...
    select.addEventListener('change', function() { setImageModel(select.value); });
  }
  ...
  // A usable text provider key is present (needed to write the headline).
  ...
  function getImageKey() {
    const provider = imageProviderFor(getImageModel());
  ...
  function renderInputs(containerId) {
    ...
    ENABLED_SERVICES.forEach(function(svc) {
      ...
      var role = svc === 'fal' ? ' — cenas (imagem)' : (svc === 'openai' ? ' — texto e imagem' : (svc === 'openrouter' ? ' — texto' : ''));
  ...
  function setupModal() {
    ...
    if (manageBtn && modal) {
      manageBtn.addEventListener('click', function() {
        ...
        // COR-041: wire the modal's own model picker too (safe no-op if absent).
        if (renderModelPicker) renderModelPicker('model-select-modal');
    ...
    function closeModal() {
      ...
      // Let the app refresh any key-dependent UI (e.g. the image-key warning).
      document.dispatchEvent(new CustomEvent('thumbnail:keys-updated'));
    }
  ...
  function init() {
    ...
    // COR-041: populate every model <select> present at init, not just the studio one.
    renderModelPicker('key-model-select');
    setupModal();
  }
  ...
  return {
    ...
    renderImageModelPicker: renderImageModelPicker
  };
})();
