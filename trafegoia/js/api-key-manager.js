...
const ApiKeyManager = (function() {
  const STORAGE_PREFIX = 'trafego-ia_apikey_';
  ...
  // OpenRouter first (preferred — one key reaches every provider in the picker,
  // Hard Rule #19), then the native single-provider keys as fallbacks.
  const ENABLED_SERVICES = ['openrouter', 'openai', 'anthropic', 'gemini'];
  ...
  // Verified against the LIVE OpenRouter ranking on 2026-07-16 (every id
  // confirmed present via GET https://openrouter.ai/api/v1/models).
  const AI_MODELS = {
    claude: {
    ...
    openai: {
      ...
      models: [
        ...
        { id: 'openai/gpt-5.6-luna', name: 'GPT-5.6 Luna (rápido)' },
    ...
    trending: {
      ...
      // Top movers pulled from the live OpenRouter ranking this build (2026-07-16).
      models: [
