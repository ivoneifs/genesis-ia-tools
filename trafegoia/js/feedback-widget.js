...
const FeedbackWidget = (function() {
  ...
  function createWidget() {
    ...
    btn.style.cssText = 'position:fixed;bottom:24px;right:24px;z-index:9999;display:flex;align-items:center;gap:6px;padding:10px 18px;background:linear-gradient(135deg,#0891b2,#0e7490);color:#fff;border:none;border-radius:100px;font-size:13px;font-weight:600;font-family:inherit;cursor:pointer;box-shadow:0 4px 20px rgba(8,145,178,.4);transition:transform .2s,box-shadow .2s;';
    ...
    modal.innerHTML =
      ...
      '<div style="position:relative;background:#13161f;border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:28px;width:90%;max-width:440px;">' +
        ...
        '<div style="display:flex;gap:8px;margin-bottom:16px;">' +
          ...
          '<button class="fb-type-btn active" data-type="feature" style="flex:1;padding:10px;background:rgba(34,211,238,.14);border:1px solid rgba(8,145,178,.4);border-radius:10px;color:#22d3ee;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">💡 Sugestão</button>' +
        '</div>' +
        ...
        '<button id="feedback-send" style="width:100%;margin-top:12px;padding:14px;background:linear-gradient(135deg,#0891b2,#0e7490);color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;">' +
    ...
    modal.querySelectorAll('.fb-type-btn').forEach(function(b) {
      b.addEventListener('click', function() {
        modal.querySelectorAll('.fb-type-btn').forEach(function(x) {
          x.style.background = x.dataset.type === 'bug' ? 'rgba(239,68,68,.05)' : 'rgba(34,211,238,.05)';
          x.style.borderColor = 'rgba(255,255,255,.08)';
        });
        b.style.background = b.dataset.type === 'bug' ? 'rgba(239,68,68,.1)' : 'rgba(34,211,238,.14)';
        b.style.borderColor = b.dataset.type === 'bug' ? 'rgba(239,68,68,.2)' : 'rgba(8,145,178,.4)';
