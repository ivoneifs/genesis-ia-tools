...
const ApiKeyManager = (function() {
  const STORAGE_PREFIX = 'transcricao-ia_apikey_';

  const AI_SERVICES = {
    openai: {
      name: 'OpenAI — transcrição (Whisper) + tradução',
      placeholder: 'sk-...',
    ...
    openrouter: {
      name: 'OpenRouter — opcional (mais modelos para tradução/limpeza)',
      placeholder: 'sk-or-...',
  ...
  // Which services this specific tool uses.
  // openai = REQUIRED (Whisper transcription engine + default translate/clean model).
  // openrouter = OPTIONAL (unlocks the full provider-grouped model picker for the
  // translate/clean step — one key reaches Claude/Gemini/DeepSeek/etc).
  ...
  // Refreshed from the LIVE OpenRouter ranking on 2026-07-21 (Hard Rule #19).
  // Used ONLY for the optional translate/clean step. Default is an OpenAI model so
  // it works with just the required OpenAI key; picking a non-OpenAI model needs an
  // OpenRouter key (the app guides the user).
  const AI_MODELS = {
    openai: {
      label: 'OpenAI (funciona com sua chave OpenAI)',
      models: [
        { id: 'openai/gpt-5.5', name: 'GPT-5.5' },
        { id: 'openai/gpt-5.5-pro', name: 'GPT-5.5 Pro' },
        { id: 'openai/gpt-5.4-mini', name: 'GPT-5.4 Mini — rápido' }
      ]
    },
    claude: {
      label: 'Claude (Anthropic) — requer OpenRouter',
      models: [
    ...
    google: {
      label: 'Google (Gemini) — requer OpenRouter',
      models: [
        { id: 'google/gemini-3.5-flash', name: 'Gemini 3.5 Flash — rápido/barato' },
        { id: 'google/gemini-3.6-flash', name: 'Gemini 3.6 Flash' },
        { id: 'google/gemini-3.1-flash-lite', name: 'Gemini 3.1 Flash Lite' }
      ]
    },
    deepseek: {
      label: 'DeepSeek — requer OpenRouter',
      models: [
    ...
    trending: {
      label: 'Em alta (OpenRouter) — requer OpenRouter',
      models: [
  ...
  // Default routes through the REQUIRED OpenAI key (native path) so translate/clean
  // work out of the box without an OpenRouter key.
